/*You are given n activities (from 0 to n-1) with their 
start and finish times. Select the maximum number of activities that can be performed by
...
int maxW(pair<int,int> A[],int s)
{
    
	sort(A,A+s,compare);
   
	int c=1;
	int LlastTime=A[0].second;
	for(int i=1;i<s;i++)
	{
        if(LlastTime<=A[i].first)
		{
			c++;
			LlastTime=A[i].second;
		}
	}
	return c;
}
